...
*/
void sendMessage(int id, int num,int lg, int lgmax, int dest, MPI_Comm comm)
{
    int buffer[4];
    buffer[0] = id;
    buffer[1] = num;
...
int main(int argc, char *argv[])
{
    ...
    // A la reception de INIT() par tous les processus 
    ...
    while (1)
    {
        ...
        if (buffer[0] == 2)
        {
            printf("noeud %d : vainqueur est %d\n ",pi,buffer[1]);
            vainqueur = buffer[1];
            if(vainqueur != pi){
                sendMessage(2,vainqueur,-1,-1 ,dest, graph_comm);
                etat = NONCONCERNE;
